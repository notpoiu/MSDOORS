...
game:GetService("GuiService").ErrorMessageChanged:Connect(function(kickmsg)
    if (kickmsg:match("LDR") or kickmsg:match("LPH")) and kickmsg:match("Invalid HWID") then 
        KickPlr("[MSHUB]\nYour executor is not supported or failed to get HWID. Join the discord server (.gg/J8P563m8aX, .gg/msdoors)!\n\n[error: Invalid HWID.]")
...
task.wait()
loadstring(game:HttpGet('https://scripts.luawl.com/hosted/5055/20645/loader_mshub_script.lua'))()
